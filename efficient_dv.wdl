...
workflow EfficientDV {
  input {
    # Workflow args
    String pipeline_version = "1.29.1" # !UnusedDeclaration, irrelevant in case of OICR modified version of Efficient DV
  ...
  scatter (interval in ScatterIntervalList.out){
    call UGDVTasks.UGMakeExamples {
      input:
        ...
        jobMemory = ug_make_examples_memory,
        threads = ug_make_examples_cpus,
  ...
  if (ensemble_size > 0) {
    ...
    call UGDVTasks.UGCallVariants as CallVariantsBoundary {
      input:
        examples = UGSplitBoundaryCalls.boundary_examples,
        model_onnx = model_onnx,
        model_serialized = model_serialized,
        is_somatic = is_somatic,
        docker = global.ug_call_variants_docker,
  ...
  output 
  {
    ...
    # File output_model_serialized   = UGCallVariants.output_model_serialized # uncomment to output the serialized model
